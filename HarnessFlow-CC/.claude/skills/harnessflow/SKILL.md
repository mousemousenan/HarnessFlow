...
2. **Agent 不能自我宣布完成。** 子代理返回的 `result` 字段是建议。任务变为 `done` 必须经过
   独立验证：路径归属核对 + 确定性 `verify` 的原始证据 + 验收标准逐条判定。
   取证与判定可以分工，判定必须由独立 `hf-verifier` 完成；编排器或 worker 不得自行放行。
   `multi_tier` 下由 verifier 独立重跑 `verify`；`single_model` 下由编排器直跑取证、
   verifier 读取原始日志判定。证据缺失、失效或无法归属时不得标 `done`。
...
| `verify` | 非交互验证命令，非空。`multi_tier` 由 verifier 独立重跑；`single_model` 由编排器直跑取证，verifier 依据原始日志独立判定 |
...
以及确定性 `verify` 由编排器直跑取证、独立 verifier 判定。
...
阶段重派上限 2（初次派发不计在内）。摘要复核失败、异常退出和看门狗停滞共用该计数。
worker 派发数量与任务修复次数在恢复时延续，新编排器只消费剩余预算。

## 运行产物

两类路径属于运行基础设施，不放进任务 `allowed_paths`，也不替代独立验证证据。
当前 hook 已豁免整个 `.git/harnessflow/`；本轮只新增使用这两类路径，不扩展 hook。

### 阶段台账

路径：`.git/harnessflow/phase_ledger.jsonl`。UTF-8 无 BOM，每行一个 JSON 对象，
序列化后不超过 500 字符，只追加不改写：
...
- `ts` 使用带时区的 ISO 8601。示例 ID 只说明格式。
- `step` 使用固定事件名；任务 ID、结果和关键产物路径放入 `detail`。
- 尚未进入批次的阶段事件将 `batch` 记为空字符串。
- 仅活动阶段编排器追加；worker、verifier 和看门狗不直接写台账。
- 台账不是任务完成证据，也不是 `04_STATE.md` 的替代品。

固定事件：`phase_started`、`scope_written`、`worker_dispatched`、`worker_returned`、
`verifier_dispatched`、`verify_command_done`、`verifier_returned`、`deliverable_written`、
`scope_cleared`、`phase_summary_ready`，以及长时等待的 `command_running` / `waiting`。

超过一个轮询周期（20 分钟）的工作，编排器必须以可返回控制权的等待方式，
每不超过 20 分钟检查自身持有的任务/命令，并记录所等对象及真实状态。
不能由看门狗代写心跳来制造活性。进程存在只说明仍存活，不证明业务持续推进。

### 验证日志

路径：`.git/harnessflow/verify-logs/<任务ID>.log`。
`single_model` 下由编排器写入原始命令输出；同任务多命令、修复重跑和阶段重派追加段落，
不覆盖旧证据。独立验证结论另起一段，与原始命令输出分开。
...
Sensor 脚本退出码只证明脚本执行结束；是否存在 `fail` 必须读取报告内容，由独立 verifier 判定。
`run-sensors.py` 正常生成报告后可能返回 0，即使报告内有失败项。
...
- worker 自测只是建议，不能把 worker PASS 当成完成。
- 主会话不运行业务 `verify`。确定性、非交互命令的取证由阶段编排器或 verifier 执行。
- `single_model`：编排器直跑 pytest / lint / typecheck / build 及有明确预期退出码的脚本，
  把原始日志交给 `hf-verifier`。需要语义判断、交互操作或人工授权的步骤保留原流程。
- `multi_tier`：仍由 `hf-verifier` 独立重跑原命令。
- `hf-verifier` 始终判定 acceptance、范围归属、产物内容、测试充分性和 Sensor 结果。
  每批必须独立 verifier PASS 才能推进。免除的只是合格证据对应的重复取证，
  不是 Adequacy Review、Sensor、UI 证据检查或内容抽查。
- 日志缺失、截断、过期、无法归属，或取证后实现变化，受影响证据失效，须重新取证或判 FAIL。
  编排器不得用一句「测试通过」放行。
...
以已有独立验证证据为准保守恢复：有通过证据的算 `done`，无证据的退回 `todo`，不猜。
台账记录不是任务完成证据，也不是 `04_STATE.md` 的替代品。

进程中断留下 `running` 状态时，绝不复用旧聊天上下文。恢复顺序固定：

1. 确认旧实例及其写入者已停止，再清理属于该实例的 `active_scope.json`。
   停止失败或 scope 归属不明时不删除、不启动第二个写入者，报告明确恢复条件。
2. 读取当前尝试的 `.git/harnessflow/phase_ledger.jsonl`，先按本次实例边界和阶段筛选，
   再从尾部定位；最后 N 行不够时继续向前读。按任务重建
   「已派发、已返回、已取证、已独立验证、待收尾」。
3. 按台账指向读取必要日志、交付和 verifier 结论，并保留 `git status`、HEAD 与路径归属核对。
   正常路径不靠文件 mtime 推断业务进度。
4. 将恢复简报交给全新阶段编排器，只做剩余动作。证据缺失的任务重新取证或验证；
   不得把 worker PASS、命令 exit 0、产物存在直接转换成 `done`。
...
台账不存在、损坏或与产物不一致时，标记「台账恢复不可用」，按保守规则核对证据并记录退化原因，
不能编造中断点。尾部半行保留原状，读取前一完整有效行；恢复追加前若文件末尾没有换行，
先只追加换行隔离残行，再追加新实例的完整事件。读取时跳过损坏行。旧阶段记录、未来时间、
缺时区或无法识别的记录不能作为当前实例新鲜心跳。

「动作完成、事件未写」时：有实现产物则优先补验证，不能仅因缺事件就全部重做；
没有证据支持的不可重复操作须报告不确定性。

主会话从状态记录继承阶段重派计数、worker 已用数量和修复次数，不从零开始。
台账有派发窗口缺口时，保守核对宿主记录，不假定缺记录等于未消费。

## 主会话看门狗

派出一个阶段编排器后，主会话建立会话级唤醒检查（间隔 20 分钟，避开整点集中触发），
绑定阶段 ID、当前后台任务标识、派发时间与本次台账边界。
实际定时、查询和停止工具以当前宿主实测可用者为准；未实测可用的名称不得写入命令
`allowed-tools`。每次派发必须使用可被状态查询和停止的执行方式。

看门狗只读检查，不修改源码、台账或运行中的 scope。检查结果记入运行记录：
实际检查时间、实例状态、活动证据、最后心跳时间、异常计数和处理动作。

每轮先处理已完成/失败的终态。仍运行的实例同时满足以下条件才算健康：

1. 当前绑定的实例未失败、未被停止。
2. 近 20 分钟有归属于本次阶段执行的文件活动，或有该实例持有的 pytest/python 等验证子进程
   在运行。检查 `.git/harnessflow/`、台账、阶段 `allowed_paths` 涉及目录及
   `docs/harnessflow/deliverables/`。
3. 当前尝试的最后有效心跳年龄 ≤40 分钟；恰好等于边界时按健康计算。

不能把其他项目的 Python 进程、旧阶段日志或看门狗自己写的检查记录当成执行活动。
子进程归属需要宿主任务信息或可核验的父子进程关系，不能只匹配进程名。
进程存在也不能豁免心跳检查。
...
| 连续第二轮不健康 | 停滞处置：再核对该实例 → 停止编排器及其仍会写文件的 worker/命令 → 确认停止后清理该实例 scope → 取消旧看门狗 → 写入中断证据与重派计数 → 按恢复简报派全新编排器并建立新看门狗 |
| 正常完成 | 取消本实例看门狗，再走摘要复核；完成通知本身不能直接标阶段 done |
| 明确 failed/killed/异常退出 | 不必等待两轮，走中断恢复并计入同一重派预算 |
| HUMAN_BLOCKER、预算用尽或最终失败 | 取消看门狗，记录状态和准确恢复条件，停止自动派发 |
| 过期唤醒仍到达 | 核对实例绑定，忽略旧实例回调，不影响后来派出的编排器 |

不得递归删除整个 `.git/harnessflow/`，不得删除台账和验证日志。
清理 scope 前必须确认 `active_scope.json` 的批次属于被终止实例。

若定时唤醒不可用，记录 O1 未启用，不声称自动保护已生效。停止或实例归属能力不足时，
不执行自动清理和重派，记录人工恢复条件。此限制不能计为完整 O1 通过。

实际处置时延从最后真实活动计到确认停滞，包含活动窗口、轮询相位和调度延迟；
本参数下不保证源提案的「≤20 分钟发现」。
